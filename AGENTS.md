<!-- repository: packages/ts/kafka | kind: PACKAGE | stack: ts-package -->

# kafka

> Repository development instructions for **kafka** (Shared Package).
> Stack: ts-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys Kafka infrastructure for TypeScript (producer/consumer, retry, DLQ).

- Repository path: `packages/ts/kafka` (relative to the Omnixys root)
...
Event envelopes and headers (tenant/correlation) are contracts. Type tests present.
